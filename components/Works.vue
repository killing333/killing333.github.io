    ...
    <ul id="list-works">
        <li v-for="item in works">
            ...
            <h3 class="work-title">{{ item.title }}
                <a v-for="(platform, key) in item.platform" v-bind:href="platform.link" target="_blank" class="platform-icon platform-icon-ios hidden-print">
...
module.exports = {
    ...
    props: ['auth'],
    firebase: {
        works: firebase.database().ref('works')
    },
    ready() {
